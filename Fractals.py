...
def triangle(back_color: str=(0, 0, 0),  # Цвет фона
             ...
             points: int=20000,  # Количество рисуемых точек
             border: int=100,  # Отступы вершин от краёв изображения
             filename: str="Треугольник Серпинского"  # Имя файла для сохранения
             ) -> None:

    ''' Построение треугольника Серинского '''
...
def carpet(back_color: tuple=(0, 0, 0),  # Цвет фона
           ...
           filename: str="Ковёр Серпинского"  # Имя файла для сохранения
           ) -> None:

    ''' Построение ковра Серинского '''
    image = Image.new('RGB', (size, size), back_color)
    drawer = ImageDraw.Draw(image)
...
def fern(back_color: tuple=(0, 0, 0),  # Цвет фона
         ...
         points: int=200000,  # Количество рисуемых точек
         border: int=100,  # Отступы вершин от краёв изображения
         filename: str="Папоротник Барнсли"  # Имя файла для сохранения
         ) -> None:
...
def tree(n: float,  # Обратный коефициент левого изгиба
         ...
         decr_koef: float=0.7,  # Коефициент уменьшения длины линии
         start_angle: float=pi/2,  # Начальный угол
         filename: str="Пифагорово дерево"  # Имя файла для сохранения
         ) -> None:
    ...
    draw(point(size / 2, size - 100), line_len, start_angle)
